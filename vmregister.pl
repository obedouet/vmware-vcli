...
#
# Modified by Olivier BEDOUET to support Folder 09/2016
...
my %opts = (
   ...
   'folder' => {
      type => "=s",
      help => "Folder of the virtual machine",
      required => 0,
   },
   'vmname' => {
      type => "=s",
...
sub register() {
   ...
   my $folder = Opts::get_option('folder');	#Added by OBT
   ...
   #new code
   if (!$folder) {
       $folder_view = Vim::get_view(mo_ref => $begin->vmFolder);
   } else {
...
# Modified to use vmx instead of vmname OBT 09/2016
sub unregister() {
   my $vmxpath = Opts::get_option('vmxpath');
   my $vm_views = Vim::find_entity_views(view_type => 'VirtualMachine',
                                         filter => {'config.files.vmPathName' => $vmxpath});
                                         #filter => {'config.name' => $vmname});
   if (!@$vm_views) {
      Util::trace(0, "\nThere is no virtual machine with vmx '$vmxpath' registered\n");
      return;
   }
   foreach (@$vm_views){
      ...
      eval {
         $_->UnregisterVM();
         Util::trace(0,"\nUnregister of VM '$vmxpath' successfully\n");
      };
      if ($@) {
         if (ref($@) eq 'SoapFault') {
            if (ref($@->detail) eq 'InvalidPowerState') {
               Util::trace(0,"\nVM '$vmxpath' must be powered off"
                            ." for this operation\n");
            }
...
sub validate {
   ...
   if ($operation) {
      ...
      if (($operation eq 'register')) {
         ...
         #   Util::trace(0, "\nMust specify parameter cluster"
         #                . " for register operation\n");
         #   $valid = 0;
         #} elsif (!Opts::option_is_set('hostname')) {
         #   Util::trace(0, "\nMust specify parameter hostname"
         #                . " for register operation\n");
         #   $valid = 0;
         #}
         if (!Opts::option_is_set('vmname')) {
            Util::trace(0, "\nMust specify parameter vmname"
                         . " for register operation\n");
            $valid = 0;
         }
...
Required. Path to vmx of the virtual machine to unregister.
...
Unregistering a virtual machine
 ...
 vmregister.pl --url https://<ipaddress>:<port>/sdk/webService
               ...
               --operation unregister --vmxpath [lazy_100gb]FC/FC.vmx
