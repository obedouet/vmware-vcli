...
#
# Modified by Olivier BEDOUET 09/2016
...
use FindBin;
# Modified by OBT
#use lib "$FindBin::Bin//usr/lib/vmware-vcli/apps/";
...
sub clone_vm {
   ...
   if(@$vm_views) {
      foreach (@$vm_views) {
	    ...
	    # Added by OBT
	    if ($folder_name) {
		    $folder = Vim::find_entity_view(view_type => 'Folder',
						filter => {name => $folder_name});
		    if (!$folder) {
			Util::trace(0, "\nFolder $folder_name not available.\n");
			return;
		    }
	    } else {
		$folder = $_->parent;
